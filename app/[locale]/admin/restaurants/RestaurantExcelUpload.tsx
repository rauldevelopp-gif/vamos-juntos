...
import { bulkCreateRestaurants } from './actions';
import { toast } from 'react-hot-toast';

export default function RestaurantExcelUpload({ onSuccess }: { onSuccess: () => void }) {
    ...
    const downloadTemplate = () => {
        ...
        XLSX.writeFile(wb, "plantilla_restaurantes.xlsx");
    };
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            reader.onload = async (evt) => {
                ...
                const res = await bulkCreateRestaurants(data);
                if (res.success) {
                    if (res.count === 0) {
                        toast.error('0 importados. Revisa el formato del archivo.');
                    } else {
                        toast.success(tr("¡Éxito! Se importaron") + ` ${res.count} ` + tr("restaurantes."));;
                        onSuccess();
                    }
                } else {
