...
function CatalogContent() {
    ...
    const categories = [
        { id: 'todos', label: t('category_todos') || 'Todos', icon: LayoutGrid },
        { id: 'Estándar', label: t('category_estandar') || 'Estándar', icon: Building2 },
        { id: 'Boutique', label: t('category_boutique') || 'Boutique', icon: Sparkles },
        { id: 'Resort', label: t('category_resort') || 'Resorts', icon: Palmtree },
        { id: 'Hostal', label: t('category_hostal') || 'Hostales', icon: BedDouble }
    ];

    const translatedHotels = useMemo(() => {
        return hotels.map(hotel => ({
            ...hotel,
            name: getTranslatedValue(hotel.name, language),
            description: getTranslatedValue(hotel.description, language),
            category: getTranslatedValue(hotel.category, language),
            city: getTranslatedValue(hotel.city, language)
        }));
    }, [hotels, language]);

    const filteredHotels = useMemo(() => {
        return translatedHotels.filter(hotel => {
    ...
    }, [translatedHotels, search, activeCategory]);
            ...
            <header className="catalog-hero">
                ...
                <div className="container hero-content-wrap">
                    <div className="hero-text-center">
                        <h1 className="text-gradient">{t('catalog_hotels_title') || 'Estancias Inolvidables'}</h1>
                        <p>{t('catalog_hotels_subtitle') || 'Encuentra los mejores hoteles, resorts y alojamientos boutique para tu próximo viaje.'}</p>
                    </div>
                    ...
                    <div className="filters-integrated">
                        <div className="filters-container glass-panel">
                            <div className="search-box">
                                ...
                                <input 
                                    type="text" 
                                    placeholder={t('search_hotel_placeholder') || '¿En qué ciudad quieres hospedarte?'} 
                                    value={search}
            ...
            <main className="container catalog-grid-section">
                {loading ? (
                    <div className="loading-state">
                        <Loader2 className="animate-spin" size={40} color="#8b5cf6" />
                        <p>{t('searching_availability') || 'Buscando disponibilidad...'}</p>
                    </div>
                ) : filteredHotels.length === 0 ? (
                    <div className="empty-results glass-panel">
                        <HotelIcon size={48} opacity={0.2} />
                        <h3>{t('no_hotels_found') || 'No encontramos hoteles'}</h3>
                        <p>{t('adjust_filters') || 'Prueba ajustando tus filtros o buscando otro destino.'}</p>
                        <button onClick={() => {setSearch(''); setActiveCategory('todos');}} className="btn-premium">{t('view_all') || 'Ver todo'}</button>
                    </div>
                ) : (
