...
export default function ForgotPasswordPage() {
    ...
    const handleSendOTP = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!email) {
            setError(isEn ? 'Enter your email address' : 'Ingresa tu correo electrónico');
            return;
        }
    ...
    const handleVerify = async (e: React.FormEvent) => {
        ...
        if (code.length < 6) { setError(isEn ? 'Please enter the complete 6-digit code' : 'Ingresa los 6 dígitos completos del código'); return; }
        if (secondsLeft === 0) { setError(isEn ? 'Code expired. Request a new one.' : 'El código expiró. Solicita uno nuevo.'); return; }
        if (!newPassword || !confirmPassword) { setError(isEn ? 'Complete password fields' : 'Completa los campos de contraseña'); return; }
        if (newPassword !== confirmPassword) { setError(isEn ? 'Passwords do not match' : 'Las contraseñas no coinciden'); return; }
        if (newPassword.length < 8) { setError(isEn ? 'The new password must be at least 8 characters' : 'La nueva contraseña debe tener al menos 8 caracteres'); return; }
        ...
        try {
            ...
            const data = await res.json();
            if (!res.ok) throw new Error(data.error || (isEn ? 'Incorrect code or error verifying' : 'Código incorrecto o error al verificar'));
            clearInterval(timerRef.current!);
            setStep('success');
        } catch (err: unknown) {
            setError(err instanceof Error ? err.message : (isEn ? 'Unknown error' : 'Error desconocido'));
        ...
        return (
            <div className="container" style={{ minHeight: 'calc(100vh - 100px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem 0' }}>
                ...
                <div className="glass-panel recover-panel">
                    ...
                    <h1 className="text-gradient" style={{ fontSize: '1.8rem', fontWeight: 800, marginBottom: '1rem' }}>
                        {isEn ? "Password Updated!" : "¡Contraseña Actualizada!"}
                    </h1>
                    <p style={{ color: 'var(--text-muted)', marginBottom: '2.5rem', fontSize: '1rem' }}>
                        {isEn ? "Your password has been successfully reset. You can now log in with your new password." : "Tu contraseña se ha restablecido correctamente. Ya puedes iniciar sesión con tu nueva contraseña."}
                    </p>
            ...
            <div className="container" style={{ minHeight: 'calc(100vh - 100px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem 0' }}>
                ...
                <div className="glass-panel recover-panel">
                    ...
                    <h1 className="text-gradient" style={{ fontSize: '1.8rem', fontWeight: 800, marginBottom: '0.5rem' }}>
                        {isEn ? "Verify Your Identity" : "Verifica tu Identidad"}
                    </h1>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem', marginBottom: '1.5rem' }}>
                        {isEn ? "We sent a code to " : "Enviamos un código a "}<strong style={{ color: 'white' }}>{email}</strong>
                    </p>
                    ...
                    <form onSubmit={handleVerify} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', textAlign: 'left' }}>
                        <div>
                            <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.85rem', color: 'var(--text-muted)', fontWeight: 600, textAlign: 'center' }}>
                                {isEn ? "Enter 6-Digit Code" : "Ingresa el Código de 6 dígitos"}
                            </label>
                    ...
                    <div style={{ marginTop: '1.5rem' }}>
                        {secondsLeft === 0 ? (
                            ...
                            <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>
                                {isEn ? "Didn't receive the code? You can resend it when the timer expires." : "¿No recibiste el código? Podrás reenviarlo cuando expire el tiempo."}
                            </p>
                        )}
                    </div>
                </div>
            ...
            <div className="glass-panel" style={{ width: '100%', maxWidth: '450px', padding: '2.5rem' }}>
                <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
                    ...
                    <p style={{ margin: 0, fontSize: '0.9rem', color: 'var(--text-muted)' }}>
                        {isEn ? "Enter the email associated with your account to receive a recovery code." : "Ingresa el correo electrónico asociado a tu cuenta para recibir un código de recuperación."}
                    </p>
                </div>
                ...
                <form onSubmit={handleSendOTP} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    <div>
                        ...
                        <div className="input-group">
                            <span className="input-icon" style={{ position: 'absolute', left: '1rem', top: '50%', transform: 'translateY(-50%)', display: 'flex', alignItems: 'center', color: 'var(--text-muted)' }}><Mail size={18} /></span>
                            <input 
                                type="email" 
                                value={email}
                                ...
                                style={{ width: '100%', padding: '1rem 1rem 1rem 2.8rem', color: 'white', background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border-glass)' }}
                            />
                        </div>
                    </div>
