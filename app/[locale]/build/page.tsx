...
import { getAttractions } from '../admin/attractions/actions';
import { getRestaurants } from '../admin/restaurants/actions';
import { getYachts } from '../admin/yachts/actions';
            ...
            try {
                ...
                if (result.success && result.data) {
                    const mapped: CatalogItem[] = (result.data as Array<Record<string, unknown>>).map((item) => {
                        let details = '';
                        const itemCity = getTranslatedValue(item.city, language);
                        const itemState = getTranslatedValue(item.state, language);
                        const itemLocation = getTranslatedValue(item.location, language);
                        const itemModel = getTranslatedValue(item.model, language);
                        const itemType = getTranslatedValue(item.type, language);
                        const itemCategory = getTranslatedValue(item.category, language);
                        const itemCuisine = getTranslatedValue(item.cuisine, language);

                        switch (type) {
                            case 'hotel': details = `${item.stars}⭐ • ${itemCity}, ${itemState} • ${item.status}`; break;
                            case 'aeropuerto': details = `${item.iata} • ${itemCity} • ${item.status}`; break;
                            case 'yate': details = `${item.brand} ${itemModel} (${item.length}) • Cap: ${item.capacity}p • ${itemLocation}`; break;
                            case 'playa': details = `${itemType} • ${itemCity} • Pop: ${item.popularity}`; break;
                            case 'atraccion': details = `${itemCategory} • ${itemCity} • ${item.recommendedTime}`; break;
                            case 'restaurante': details = `${itemCuisine} • ${itemCity} • ${item.priceRange}`; break;
                        }
                        return {
                            id: item.id,
                            name: getTranslatedValue(item.name, language),
                            type: type,
                ...
                <div className="flyer-body">
                    ...
                    {pkg.driverId && (
                        <div className="flyer-driver-section">
                            <div className="driver-label">{t('assigned_driver') || 'CHOFER ASIGNADO'}</div>
                    ...
                    <div className="flyer-footer">
                        <div className="price-box">
                            <span className="label">{t('total_price_label') || 'PRECIO TOTAL'}</span>
                            <span className="value">${pkg.total.toLocaleString()} <small>USD</small></span>
                        </div>
                        <div className="contact-info">
                            <p>{t('reserve_concierge') || 'Reserva con tu Concierge VIP'}</p>
                                ...
                                <div className="fields">
                                    <div className="field-group">
                                        ...
                                        <div style={{ 
                                            ...
                                            <div style={{ background: 'rgba(245, 158, 11, 0.1)', color: '#f59e0b', padding: '0.6rem', borderRadius: '50%', display: 'flex' }}>
                                                <Clock size={20} />
                                            </div>
