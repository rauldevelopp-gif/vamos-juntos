...
import { bulkCreateAirports } from './actions';
import { toast } from 'react-hot-toast';

export default function AirportExcelUpload({ onSuccess }: { onSuccess: () => void }) {
    ...
    const downloadTemplate = () => {
        const templateData = [
            {
                name: 'Ejemplo: Aeropuerto Internacional de Cancún',
                location: 'Carretera Cancún - Chetumal Km 22',
                iata: 'CUN',
                city: 'Cancún',
                state: 'Quintana Roo',
                status: 'Operativo',
                coordinates: '21.0400,-86.8736'
            }
        ];
        
        const ws = XLSX.utils.json_to_sheet(templateData);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Aeropuertos");
        XLSX.writeFile(wb, "plantilla_aeropuertos.xlsx");
    };
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            reader.onload = async (evt) => {
                ...
                const res = await bulkCreateAirports(data);
                if (res.success) {
                    if (res.count === 0) {
                        toast.error('0 importados. Revisa que los IATA no estén repetidos en el sistema.');
                    } else {
                        toast.success(tr("¡Éxito! Se importaron") + ` ${res.count} ` + tr("aeropuertos."));;
                        onSuccess();
                    }
                } else {
