...
const mapApiToFrontend = (apiPkg: { 
  ...
  const defaultDesc = locale === 'en' 
    ? 'Exclusive experience designed for you.' 
    : 'Experiencia exclusiva diseñada para ti.';

  return {
    id: apiPkg.id,
    name: getTranslatedValue(apiPkg.name, locale) || (locale === 'en' ? 'Unnamed Package' : 'Sin nombre'),
    description: getTranslatedValue(apiPkg.description, locale) || defaultDesc,
    image: apiPkg.image || '/mexico_luxury_ruins_hero_1778020263723.png',
    duration: apiPkg.duration || (locale === 'en' ? '8 Hours' : '8 Horas'),
    startTime: apiPkg.start_time || '08:00',
    price: apiPkg.price || 0,
    maxPassengers: apiPkg.vehicle?.capacity || 8,
    pickup: {
        id: 1,
        name: getTranslatedValue(apiPkg.pickup, locale) || (locale === 'en' ? 'Departure point' : 'Punto de partida'),
        type: 'airport'
    },
    dropoff: {
        id: 2,
        name: getTranslatedValue(apiPkg.dropoff, locale) || (locale === 'en' ? 'Destination point' : 'Punto de destino'),
        type: 'hotel'
    },
    vehicle: {
        id: apiPkg.vehicle?.id || 1,
        name: getTranslatedValue(apiPkg.vehicle?.model, locale) || 'Luxury SUV',
        type: 'Premium',
  ...
  const [activeCategory, setActiveCategory] = useState<string>('todos');
    ...
    useEffect(() => {
        if (reserveId && packages.length > 0) {
            ...
            if (pkg) {
                setBookingPkg(mapApiToFrontend(pkg, language));
            }
        }
    }, [reserveId, packages, language]);

    const categories = [
        { id: 'todos', label: isEn ? 'All' : 'Todos', icon: <Sparkles size={16} /> },
        { id: 'yate', label: isEn ? 'Yachts' : 'Yates', icon: <Anchor size={16} /> },
        { id: 'restaurante', label: isEn ? 'Gourmet' : 'Gourmet', icon: <Utensils size={16} /> },
        { id: 'playa', label: isEn ? 'Beaches' : 'Playas', icon: <Palmtree size={16} /> },
        { id: 'hotel', label: isEn ? 'Hotels' : 'Hoteles', icon: <Hotel size={16} /> }
    ];

    const mappedPackages = useMemo(() => {
        return packages.map(pkg => mapApiToFrontend(pkg, language));
    }, [packages, language]);

    const filteredPackages = useMemo(() => {
        return mappedPackages.filter(pkg => {
            const nameStr = pkg.name || '';
            const descStr = pkg.description || '';
            const matchesSearch = nameStr.toLowerCase().includes(search.toLowerCase()) || 
                                 descStr.toLowerCase().includes(search.toLowerCase());
            
            const hasCategory = activeCategory === 'todos' || pkg.items.some((item: { type?: string }) => item.type === activeCategory);
    ...
    }, [mappedPackages, search, activeCategory, driverIdParam]);
            ...
            <header className="catalog-hero">
                ...
                <div className="container hero-content-wrap">
                    <div className="hero-text-center">
                        <h1 className="text-gradient">{isEn ? "Explore Luxury Caribbean" : "Explora el Caribe de Lujo"}</h1>
                        <p>{isEn ? "Find experiences meticulously designed to exceed your expectations." : "Encuentra experiencias meticulosamente diseñadas para superar tus expectativas."}</p>
                    </div>
                    ...
                    <div className="filters-integrated">
                        <div className="filters-container glass-panel">
                            <div className="search-box">
                                ...
                                <input 
                                    type="text" 
                                    placeholder={isEn ? "Where would you like to go?" : "¿A dónde quieres ir?"} 
                                    value={search}
                        ...
                        <div className="results-count-mini">
                            {isEn ? "Showing " : "Mostrando "}<strong>{filteredPackages.length}</strong> {isEn ? "luxury experiences" : "experiencias de lujo"}
                        </div>
                    </div>
                </div>
            </header>
            ...
            <main className="container catalog-grid-section">
                {loading ? (
                    <div className="loading-state">
                        <Loader2 className="animate-spin" size={40} color="#8b5cf6" />
                        <p>{isEn ? "Syncing luxury catalog..." : "Sincronizando catálogo de lujo..."}</p>
                    </div>
                ) : filteredPackages.length === 0 ? (
                    <div className="empty-results glass-panel">
                        <Sparkles size={48} opacity={0.2} />
                        <h3>{isEn ? "No matches found" : "No encontramos coincidencias"}</h3>
                        <p>{isEn ? "Try adjusting your filters or searching for something else." : "Prueba ajustando tus filtros o buscando algo diferente."}</p>
                        <button onClick={() => {setSearch(''); setActiveCategory('todos');}} className="btn-premium">{isEn ? "View all catalog" : "Ver todo el catálogo"}</button>
                    </div>
                ) : (
                            ...
                            >
                                ...
                                <div className="card-body">
                                    <div className="card-category-row">
                                        <span className="card-badge">{isEn ? "Exclusive" : "Exclusivo"}</span>
                                    ...
                                    <div className="card-footer" onClick={e => e.stopPropagation()}>
                                        <a 
                                            href={`https://wa.me/${pkg.driver?.phone?.replace(/\D/g, '') || '529981234567'}?text=${encodeURIComponent(
                                                isEn 
                                                    ? `Hello, I need more information about the package: ${pkg.name}.\nYou can view it here: ${typeof window !== 'undefined' ? window.location.origin : ''}/packages?reserve=${pkg.id}`
                                                    : `Hola, necesito más información sobre el paquete: ${pkg.name}.\nPuedes verlo aquí: ${typeof window !== 'undefined' ? window.location.origin : ''}/packages?reserve=${pkg.id}`
                                            )}`}
                                            target="_blank"
                ...
                <div className="container">
                    <div className="custom-promo glass-panel">
                        <div className="promo-info">
                            <h2>{isEn ? "Looking for something completely unique?" : "¿Buscas algo totalmente único?"}</h2>
                            <p>{isEn ? "Use our custom package builder to craft your dream trip step by step." : "Usa nuestro constructor personalizado para armar el viaje de tus sueños paso a paso."}</p>
                        </div>
                        <Link href="/build" className="btn-premium promo-btn">
                            {isEn ? "Build Custom Tour" : "Armar Tour Personalizado"}
                        </Link>
                    </div>
                </div>
            </footer>
