        ...
        <main>
            ...
            <section className="hero-banner" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '50vh', position: 'relative' }}>
                <Image 
                    ...
                    alt={isEn ? "Paradise beaches" : "Playas paradisiacas"} 
                    fill 
                ...
                <div className="container" style={{ position: 'relative', zIndex: 2, textAlign: 'center', padding: '0 2rem' }}>
                    <h1 className="heading-1 float-animation" style={{ fontSize: '3.5rem', marginBottom: '1rem', textShadow: '0 10px 30px rgba(0,0,0,0.5)' }}>
                        {isEn ? "Coastal " : "Paraísos "}<span className="text-gradient">{isEn ? "Paradises" : "Costeros"}</span>
                    </h1>
                    <p style={{ fontSize: '1.25rem', color: 'rgba(255,255,255,0.9)', maxWidth: '800px', margin: '0 auto', textShadow: '0 4px 10px rgba(0,0,0,0.5)' }}>
                        {isEn 
                            ? "Discover the most stunning beaches, where the sun shines eternally and the waves tell stories of golden sands."
            ...
            <div style={{ padding: '2rem 0 5rem 0', background: 'var(--background)' }}>
                <EntityGrid 
                    title={isEn ? <>Explore Your <span className="text-gradient">Destination</span></> : <>Explora tu <span className="text-gradient">Destino</span></>} 
                    subtitle={isEn 
                        ? "From hidden bays to vibrant and exclusive beaches."
                        : "Desde bahías escondidas hasta playas vibrantes y exclusivas."
                    } 
                    items={beaches} 
                />
            </div>
